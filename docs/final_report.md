...
| 语言 | 状态 | 说明 |
|------|------|------|
| **Python** | 已完成 | 185 个仓库，6.8M 样本，全量产出 |
| **Go** | 开发中 | parse 机制需适配（Go 测试文件命名 `_test.go`，包结构不同）；filter 中 `--python-only` 需替换为 `--go-only` |
| **TypeScript** | 开发中 | 需处理 `.ts`/`.tsx` 文件分类、Jest/Vitest 测试检测逻辑 |
| **Java** | 计划中 | Maven/Gradle 项目结构解析、`src/test/` 路径匹配 |

多语言适配核心工作：
1. `parse_commits.py`: 语言检测逻辑（文件后缀 + 项目配置文件识别）
2. `filter_commits.py`: 语言专用的文件分类规则（src vs test vs docs）
3. `generate_tasks.py`: localization 的目录结构适配（Go 的 flat package 与 Python 的 nested module 不同）
...
| 数据源 | 建议占比 | 说明 |
|--------|---------|------|
| localization | 30-40% | 虽然原始占比 58%，建议 downsample 至 30-40%，避免过拟合定位任务 |
| edit_generation | 25-30% | 核心编辑能力，质量最高，建议 upsample |
| commit_message | 15-20% | 代码理解辅助任务 |
| test_writing | 5-10% | 量少但重要，建议 upsample 3-5x |
| 通用语料（代码/文本） | 10-20% | 防止灾难性遗忘，混入 general code corpus |

### 混合策略要点

- **按 repo 做 cap**: 单个 repo 占比不超过 5%（尤其 odoo 需 downsample）
- **task-specific 训练**: 如果目标是 SWE-bench，优先 localization + edit_generation
- **curriculum 策略**: 先 commit_message → localization → edit_generation → test_writing（由易到难）
- **与通用语料混合**: 建议加入 The Stack / StarCoder 训练集的 Python 部分，保持通用编码能力
...
| 限制 | 影响 | 缓解方案 |
|------|------|---------|
| **test_writing 量少** (27K) | 测试生成能力训练不充分 | 计划扩展至 relaxed/max filter 级别，预计可扩至 100K+ |
| **MinHash 去重未做** | 可能存在近似重复（微小变体） | 计划加入 MinHash (n-gram=5, threshold=0.8) 模糊去重 |
| **localization 占比过高** (58%) | 训练时需手动 downsample | 建议训练侧按类型 sample rate 调整 |
| **仅 Python** | 不覆盖 Go/TS/Java 等 | 多语言支持开发中（见上节） |
| **bug_detection/code_review 质量差** | 2 种任务类型不可用 | 需要 LLM-based 重写或完全重新设计生成逻辑 |
| **无 execution 验证** | 无法保证生成的编辑/测试实际可运行 | 考虑接入 sandbox 做 compile/run 验证 |

### 未来改进计划

1. **test_writing 扩量**:
   - 当前仅在 strict filter 下生成（需同时有 src + test 改动）
   - 计划放宽至 relaxed/max：只要 commit 包含 test 文件改动即可生成
   - 预期产量: 80K-150K

2. **MinHash 模糊去重**:
   - 当前仅做 MD5 精确去重
   - 计划用 datasketch 库做 MinHash LSH
   - 参数: n-gram=5, num_perm=128, threshold=0.8
   - 预期额外去重: 10-15%

3. **localization downsample / rebalance**:
   - localization 占总量 58%，训练时建议 downsample 到 30-40%
   - 或设置 per-type sample weight

4. **bug_detection 重建**:
   - 当前版本因 commit message 直接拷贝导致质量差
   - 需用 LLM（如 Claude Haiku）重写 bug 描述
   - `scripts/synthesize_bug_descriptions.py` 已有初步实现

5. **多语言扩展**: Go → TypeScript → Java（按优先级）

6. **质量分级**: 对 edit_generation 按 diff 复杂度分层，训练时可按难度 curriculum
